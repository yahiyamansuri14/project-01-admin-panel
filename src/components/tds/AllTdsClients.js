...
export default function AllTds() {
  ...
  useEffect(() => {
    async function verifyToken() {
      ...
      axios({
        method: "GET",
        url: "http://3.108.219.92:3800/v1/user/tds/getall/admin",
        data: {},
        headers: {
                ...
                <tbody>
                  {tds_list.map(tds => (
                    <tr>
                      ...
                      {/* <td className='fw-bolder'>{tds.status ? tds.status : null}</td> */}
                      <td className='fw-bolder color-blue cursor-pointer'>Edit</td>
                    </tr>
                  ))}
                   {/* onClick={() => navigate(`/tds/client/view/${tds._id  } `, {state:{ tds}})} */}
                </tbody>
              </table>
            </div>
          </div>
